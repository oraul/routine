...
EOF
  ...
  printf '@test "routine-widget runs and prints ok" { [ -f "$BATS_TEST_FILENAME" ]; }\n' > "$sroot/test/widget.bats"
  printf '@test "routine-verdict exits on its count" { [ -f "$BATS_TEST_FILENAME" ]; }\n' > "$sroot/test/verdict.bats"
}
...
@test "a dead test pointer fails, a pointer that never names the script too" {
  ...
  printf '@test "the suite claims something unrelated" { [ -f "$BATS_TEST_FILENAME" ]; }\n' > "$sroot/test/widget.bats"
  lint
  [ "$status" -ne 0 ]
  case "$output" in *routine-widget*widget.bats*) ;; *) false ;; esac
}
