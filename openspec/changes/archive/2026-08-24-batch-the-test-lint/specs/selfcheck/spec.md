...
#### Scenario: Scanning cost scales with suites, not tests
- **WHEN** a corpus of two suites holding thirty clean tests each is
  linted with every external tool launch tallied through a PATH shim
- **THEN** the lint exits 0 having launched at most thirty tools —
  a bound per-test judging exceeds by an order of magnitude
