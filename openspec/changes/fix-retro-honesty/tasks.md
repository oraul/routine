...
- [x] 1.1 Red→green: expected non-zero exits (`tdd.red`,
...
- [x] 2.1 Red→green: block pairing ignores refused `ticket.block` and
      `ticket.unblock` records, so no phantom block survives

## 3. The queue ranks by pain, not by luck

- [x] 3.1 Red→green: the deepening queue orders by failure count with
